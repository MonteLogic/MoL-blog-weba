...
// Configuration
const SCHEMA_PATH = path.join(process.cwd(), 'blog-schema/categories-schema.json');
const POSTS_PATHS_JSON = path.join(process.cwd(), 'blog-schema/file-paths/markdown-paths.json');
...
function validateCategories() {
  ...
  console.log(`Loaded ${validCategorySlugs.size} valid categories from schema.`);
  ...
  console.log(`Checking ${postPaths.length} posts...`);
  ...
  postPaths.forEach((filePath) => {
    ...
    slugsToValidate.forEach((slug) => {
      ...
      if (!validCategorySlugs.has(slug)) {
        console.error(`❌ INVALID CATEGORY SLUG: "${slug}" in file: ${filePath}`);
        console.error(`   Allowed categories: ${Array.from(validCategorySlugs).join(', ')}`);
        errorCount++;
      }
    });
  });
