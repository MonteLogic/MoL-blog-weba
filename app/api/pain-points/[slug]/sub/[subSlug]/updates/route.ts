...
export async function POST(
  ...
  try {
    ...
    const path = `posts/categorized/pain-points/${slug}/sub-pain-points/${subSlug}/updates/${filename}`;
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        message: `Add update to sub pain point ${subSlug} in ${slug}`,
    ...
    console.error('Error creating sub pain point update:', error);
    return NextResponse.json(
